...
function buildLootImportTab() {
  ...
  fetchPastedLootSummary(renderLootImportForm);
}
...
function renderLootImportForm(summary, preservedStatus) {
  var el = document.getElementById('loot-sub-import');
  if (!el) return;
  ...
  var html = '<div class="signup-officer-panel" style="margin-bottom:1rem;">';
  ...
  if (count > 0) {
    ...
    html += '<button class="btn btn-danger" onclick="confirmClearAllLoot()">Clear All Loot History</button>';
    html += '</div>';
    ...
    html += '<p class="signup-officer-note" style="margin-top:0.35rem;">No loot history imported yet.</p>';
  }
  html += '</div>';

  var seasonName = (window.DATA && DATA.seasonName) ? DATA.seasonName.trim() : '';
  html += '<div class="signup-officer-panel">';
  ...
  if (preservedStatus) {
    var statusEl = document.getElementById('lootImportStatus');
    if (statusEl) {
...
function submitLootImport() {
  ...
  sendLootChunks(season, rows, 0, 0, 0, function(written, skipped) {
    ...
    fetchPastedLootSummary(function(summary) {
      renderLootImportForm(summary, { color: 'var(--heal)', text: msg });
    });
  });
}
...
function executeClearAllLoot() {
  ...
  window[cbName] = function(result) {
    ...
    if (result && result.success) {
      fetchPastedLootSummary(function(summary) {
        renderLootImportForm(summary, { color: 'var(--heal)', text: 'All loot history cleared.' });
      });
    } else {
      setLootImportStatus('Clear failed.', 'var(--melee)');
    }
  };
  var script = document.createElement('script');
  script.onerror = function() { delete window[cbName]; setLootImportStatus('Clear failed.', 'var(--melee)'); };
