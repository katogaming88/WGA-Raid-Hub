...
### Added
- Contextual help tips on the officer dashboard: every major tab now has a `?` button next to its section header that toggles an inline tip panel explaining the workflow. Covered sections: Roster, Priority, Loot Import, Attendance, Signups, BiS Submissions, M+ Exclusions, Received Item Requests, and Season Settings (which also includes the full season reset checklist).
