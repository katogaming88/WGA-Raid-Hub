...
// the seed and on production alike; #938 retired seasonName the same way, and
// #1269 the two typed dates, whose start the database derives. One definition per signup function: the
// old signatures were dropped, so PostgREST never has two candidates.
describe('after #934', () => {
  it('no team_settings.config row carries a key a season table now holds', async () => {
    await withTxn(async ({ q }) => {
      const res = await q(
        "select count(*)::int as n from public.team_settings where config ?| array['activeSignupSeason', 'signupsOpen', 'wishlistOpen', 'seasonName', 'seasonStart', 'seasonEnd']"
      );
      expect(res.rows[0].n).toBe(0);
    });
  });
