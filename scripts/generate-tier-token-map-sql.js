...
export function tierTokenMapRows(classItems, season = SEASON) {
  const rows = [];
  ...
  for (const { class: className, armor_type, items } of classItems) {
    for (const [slot, resolvedName] of Object.entries(items)) {
      ...
      rows.push(
        `  (${sqlString(season)}, (select id from items where lower(name) = lower(${token})), ${sqlString(className)}, ` +
...
function main() {
  ...
  const sql = `-- Generated by scripts/generate-tier-token-map-sql.js. Run after both the\n-- token items (normal season import) and tier_resolved_items.csv are\n-- already imported into items -- this matches both sides by name. It adds\n-- ${SEASON}'s rows next to earlier seasons' rather than replacing them (#1108).\ninsert into public.tier_token_map (season, token_item_id, class, resolved_item_id)\nvalues\n${rows.join(',\n')};\n`;
