...
//
// The sheet speaks difficulty (Heroic/Mythic); the table stores the item
// track (Hero/Myth, decided on #343). Champion never appears here: that
// loot is handled by loot council outside the priority system.
//
...
export function parsePriority(rows, label = 'Priority Order') {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!item) continue;
    const rawDiff = String(row[0] || '')
      .trim()
      .toLowerCase();
    const track = rawDiff ? { heroic: 'Hero', mythic: 'Myth' }[rawDiff] : 'Hero';
    if (!track) {
      throw new Error(`${label} row ${i + 1}: difficulty ${JSON.stringify(row[0])} (sheet allows Heroic/Mythic)`);
    }
    for (let c = RANK_START_COL; c < row.length; c++) {
      const name = String(row[c] || '').trim();
      if (!name) continue;
      entries.push({ item, track, rank: c - RANK_START_COL + 1, name });
    }
  }
  return entries;
}
...
export function prioritySql(teamId, entries, registry, season, knownItems) {
  ...
  const sql = insertStatement(
    'priority_order',
    ['team_id', 'season', 'item_id', 'track', 'rank', 'player_id'],
    valueRows,
    'on conflict (team_id, season, item_id, track, player_id) do nothing'
  );
