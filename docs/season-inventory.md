...
- **Writers.** `import_rclc_loot(p_team_id, p_season, p_rows)` from `submitLootImport()` (`js/tabs/tab-loot-import.js`) and `qaSubmitLoot()` (`js/officer-quick-actions.js`), both with `seasonCodeForDisplay(DATA.seasonName)`. The #320 importer (`scripts/import/tables/loot.js`) takes the season from the pasted row or, for legacy rows, from `--seasons` date ranges, as the code since #938. `clearLootDataSupabase()` (`js/tabs/tab-admin.js`) deletes the team's rows; `submitLootReassign()` moves a row between players and keeps the stamp.
