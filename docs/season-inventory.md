...
- **Next tier, nothing changed.** The first sync for a team whose `seasonName` is the new name creates the new tier's rows; a team with no name lands them under `Unknown`. Until a row exists, Season View cannot name the new tier and the scope check fails open (every raid item counts). Converted in #933 (2026-09-20): the stamp is `current_season()`, so the first sync after the tier's migration creates the rows for every team.
