...
- **Writers.** `saveSeasonStart()`, `saveSeasonEnd()` (`js/tabs/tab-season.js`); until #938's third pull request `archive_current_season()` blanked both, and `close_season()` leaves them alone; `unarchive_season()` restores both.
