...
- **Writers.** `bisSlotPickItem()` (`js/tabs/tab-bis.js`) inserts a row stamped with `resolveSeasonView()`, which is Season View when an officer set one, else the team's `seasonName`. `20260725135340_bis_items_item_preferences_season.sql` added the column and backfilled existing rows from each team's `seasonName`. `archive_current_season()` deletes every row for the active roster at rollover, after snapshotting them into `seasonHistory`. `removeBisListItem()` deletes one row. `toggleBisItemObtained()` and `applyRaiderIoTierSync()` (`js/common.js`) update `obtained` only, and the `restrict_bis_items_update_to_obtained` trigger refuses any update that changes `season`. The #320 importer (`scripts/import/tables/bis.js`) predates the column.
- **Readers.** `fetchSupabaseBisItems()` and `mapSupabaseBisItems()` (`js/common.js`) carry it as `entry.season`, and `isItemInSeasonScope(entry.item, entry.season)` filters on it in `buildBisListsTab()` and `bisSlotBuckets()` (`js/tabs/tab-bis.js`), `refreshBisCompletion()` and `renderProfile()` (`js/common.js`), and `wishlistCompleteness()` (`js/wishlist.js`): a placeholder row is in scope when its season equals `resolveSeasonView()`, a real item by its raid's zone regardless of the stamp. No SQL reader: `generate_priority_order()` and `wishlist_setup_status()` join the table without the column, and the archive snapshot carries the item and `obtained`, not the season.
...
- **Next tier, nothing changed.** A team that has not clicked Start New Season keeps stamping the old name, or null; nothing notices because nothing reads it. Converts in #937.
...
- **Next tier, nothing changed.** The first sync for a team whose `seasonName` is the new name creates the new tier's rows; a team with no name lands them under `Unknown`. Until a row exists, Season View cannot name the new tier and the scope check fails open (every raid item counts). Converts in #933.
...
- **Meaning.** Cycle: an officer's planning override for the catalog, wishlist, BiS and priority scope, separate from `activeSignupSeason` on purpose. **Next tier, nothing changed.** Cannot name the new tier until `raid_zones` holds a row for it. Converts in #933.
