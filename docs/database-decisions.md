...
- Still open for #1045 before creation is opened to everyone: guild officers act in every guild, grants refuse a second guild, and one person can create unlimited guilds.
...
- **The switch must stay off until #1045 lands.** A second guild is still unsupported: `only_guild_id()` (used by `admin_grant`) raises once a second guild exists, and `admin_create_team()` refuses to guess. The switch is the place to hold that line; the site admin's card on the create page says so.
