...
- **`create_guild(name, region, realm, team_name)`** makes the guild, its first team, the `team_settings` row and the caller's `team_leader` membership in one step, and answers with the two address keys. Both keys are the random defaults; a readable key stays a site admin's to give (#1114). It refuses a missing Discord (the leader's membership is keyed on it, as in `team_invite_link_join()`), a blank or over-long name, a region outside us/eu/kr/tw, a blank realm, and a guild or team name already taken (case-insensitive). It writes its own `audit_log` row, as the other self-service RPCs do.
