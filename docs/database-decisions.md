...
- **The five lifecycle RPCs (`boe_record_listing`, `boe_record_sale`, `boe_mark_paid`, `boe_retire`, `boe_revert`) insert their own `audit_log` row directly**, in the same transaction as the mutation, rather than calling `write_audit_log()`. They are already `SECURITY DEFINER` running as postgres, so nothing about their own gate changes. `boe_revert` gets an entry it never had. Wording matches what the client used to send byte for byte (`format_boe_gold()`, a small SQL helper mirroring `formatGold()` in `js/boe-manage.js`, reproduces the thousands-separator formatting).
...
Shipped: 20260922000521_team_season_start.sql
