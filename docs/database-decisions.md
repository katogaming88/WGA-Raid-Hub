...
## 2026-07-18 -- Season-code to display-name mapping (#341): stays a hardcoded frontend map, not a table or settings key
...
- **Kept as a hardcoded `SEASON_LABELS` map in `js/common.js`** (`seasonCodeForDisplay()` for the reverse direction), rather than a dedicated `seasons` table or a `settings.config` key -- this was already the de facto implementation since #209, just never formally decided. Season codes change a handful of times a year at most, and a new one only becomes reachable once a code change adds it here anyway (nothing derives `MID2` on its own from `MID1`) -- a DB-backed mapping would need the same code change to actually _use_ the new row, so it buys no operational win over just editing the map directly.
- **New runbook, documented in the code comment**: when starting a new season (after `archive_current_season`, before typing the new display name into Season Settings), add its code to `SEASON_LABELS` in the same PR. `seasonCodeForDisplay()` only matches an _exact_ value -- if the code is missing, every write that resolves `p_season` from `DATA.seasonName` (RCLC import, priority generation, the export string) silently falls through to storing the full display string as the season key instead of a short code. Still internally self-consistent (nothing breaks), but inconsistent-looking next to `MID1` and defeats the compact-key design -- worth catching before, not after, a season's data starts accumulating under the wrong key shape.
