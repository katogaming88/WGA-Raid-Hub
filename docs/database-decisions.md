...
## 2026-09-24 -- Creating a guild and its first team, behind a switch a site admin flips (#1226)

Shipped: 20260923204829_guild_creation.sql

The site front page (#1226) gets a create-your-guild form. Kat decided (2026-09-24) not to open guild creation yet, but to build it now so opening it later is one switch, not another release.

- **`site_settings.guild_creation_open`, default false.** While false only a site admin can create a guild; once a site admin sets it true, anyone signed in with Discord can. `admin_set_guild_creation_open()` (site admin only, audit-logged) is the only write path; `guild_creation_open()` reads it and is granted to `anon`, because the signed-out front page decides whether to offer the form before sign-in.
- **`create_guild(name, region, realm, team_name)`** makes the guild, its first team, the `team_settings` row and the caller's `team_leader` membership in one step, and answers with the two address keys. Both keys are the random defaults; a readable key stays a site admin's to give (#1114). The team name is optional (Kat, 2026-09-24: a guild with one team does not name it): left blank, the team takes the guild's name. It refuses a missing Discord (the leader's membership is keyed on it, as in `team_invite_link_join()`), a blank or over-long guild name, an over-long team name, a region outside us/eu/kr/tw, a blank realm, and a guild or team name already taken (case-insensitive). Two callers with the same name at the same instant get the same message, not a raw duplicate-key error. A blank team name that would collide as the guild's name says so, since the caller never typed it. It writes its own `audit_log` row, as the other self-service RPCs do.
- **The switch must stay off until #1045 lands.** A second guild is still unsupported: `teams.name` is unique across all guilds, `only_guild_id()` (used by `admin_grant`) raises once a second guild exists, and `admin_create_team()` refuses to guess. The switch is the place to hold that line; the site admin's card on the create page says so.
