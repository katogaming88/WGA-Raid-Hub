...
- **A pick with no season belongs to no tier, where the page used to fail open on it.** The app's placeholder checks read a null season as the tier being planned, and the site read every row a raider held whatever its season; both now match the row's season exactly, including when no tier resolves and the page asks for the rows with none. That is the same question the write gate asks, so the page can no longer offer an edit the database will refuse. On production the rows this moves are the 9 with no season, all on 2 archived characters, whose profiles no raider edits.
- **No season resolved is not a season, and the page goes read-only.** Scoping the page to one season made it depend on resolving one, and `fetchSupabaseSeasons()` warns and returns an empty list on failure, so a transient failure reads as "no season" rather than as an error. The page would then ask for only the seasonless picks, a raider's wishlist would look empty, and the one raider who can still edit there (one an officer allowed, since the team switch reads closed without a tier) would write a second seasonless row beside the pick they already held, which the widened key permits. So with no season resolving the page narrows nothing and refuses every write, through `wishlistEditableNow()`, which carries its nine call sites and the read-only note already written for the closed case. `isItemInSeasonScope()` moved with it: its raid branch already failed open there while its placeholder branch compared every stamped pick against the empty string and hid them all, so the page showed a different wishlist rather than all of it. Both fail open now when there is no season to compare against, and the strictness once a Season View is pinned is untouched. The app reaches the same state when no tier has started and no pin is set, since its season read errors rather than emptying, and its editor and Loot priority card stop narrowing there too. The root cause, a read that cannot report failure distinctly from emptiness, is filed separately.
- **The tripwire moves rather than going, because the key was doing work for readers that do not name a season.** It used to fail if a tier started after MID2 while the key had no season; that order is kept, and the cases replacing it assert the key carries the season and that a raider can file the same item and slot in two tiers. But while a pick was unique by item and slot, a read of every row a raider holds for an item could only be the tier in play, which is what kept two officer-side readers correct without either naming a season: `generate_priority_order()` takes the strongest status across those rows, so a BiS mark left in another tier would make a raider a candidate for this one at the top of the order even where their row here says pass, and `wishlist_setup_status()` counts them, so a slot filled in another tier reads as filled. The tripwire is re-aimed at those two and deletes itself when #936's remaining piece puts the season on them.
- **Two tiers of picks no longer need a new tier to reach.** An officer can pin Season View to MID1 and open that tier's wishlist switch, which `set_team_season()` still allows for an ended tier (refusing one is listed as not moved on the #939 entry). The key is what blocked that before. The seasons half of the tripwire is the coarse guard, kept because a new tier is the ordinary way this arrives, and the two reader checks are the precise ones.
