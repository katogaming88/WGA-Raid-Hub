...
Shipped: 20260921192826_cleanups_read_current_season.sql
...
- **Four pull requests, in this order, each closing one concern.** First, the two stale-priority cleanups read `current_season()` (this line's migration). Second, the importer stamps codes (no migration). Third, `close_season()` replaces the archive pair. Fourth, the key leaves every config row and every site and app reader takes the tier. The third and fourth carry a migration each, named on the `Shipped:` line as they land; the fourth closes the issue. The order is the dependency: the old archive names its history entry from the key and the old Season tab writes it back after an archive, so the archive is replaced before the key goes, and the cleanups stop reading it before the strip would turn their read null.
