...
- **The database holds the wishlist switch, not only the page.** A trigger, `restrict_item_preferences_to_open_wishlist()`, refuses a raider's insert, update or delete of their own row unless the team's `team_seasons` row for the row's season has `wishlist_open`, or the character has `players.wishlist_allowed`. That is the test both Wishlist pages already applied before offering editing, so the switch now holds for a page that has not re-read it and for a direct call. An update is checked on both sides, so a row can neither be edited in a closed season nor moved into one. A row with no season has no switch and opens only for the override, as on both pages. Read on production before it shipped (2026-09-24): Hellfire Rollers has MID2 open, Phoenix has it closed with seven raiders on the override, Immolation has no row, and every Phoenix insert since the switches moved onto `team_seasons` came from a raider who holds the override, so the gate refuses nothing the pages allowed.
