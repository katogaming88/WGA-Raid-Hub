...
- `seasons` (one row per raid tier with its dates, [#932](https://github.com/katogaming88/WGA-Raid-Hub/issues/932)): the rows are in the migrations that added each tier, so a schema rebuild brings them back. Every `season` column is a foreign key to it, so a selective restore of a table that carries a season needs its tier rows in place first.
...
- `team_seasons` (a team's signups-open and wishlist-open switches per tier, [#939](https://github.com/katogaming88/WGA-Raid-Hub/issues/939); officer-set, and the only record of which tiers a team has opened)
- `team_invite_links` (a team's active invite code and its expiry, [#1264](https://github.com/katogaming88/WGA-Raid-Hub/issues/1264); an officer generates it in team settings and nothing else records it, so a restore that loses the row leaves a shared link dead until an officer resets it). No `EMPTY_CHECK` floor: a team that has never made a link legitimately has no row.
- `site_settings`
...
- `account_preferences` (per-account UI state: the "no character" claim-prompt dismissal and the notification bell's cleared-through marker, [#940](https://github.com/katogaming88/WGA-Raid-Hub/issues/940); losing it only makes a dismissed prompt or cleared notifications reappear)
- `people` (one row per human, [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942); `team_members` and `guild_grants` point at it through `person_id`, so it restores before them. Rebuildable from their Discord ids and `auth.identities` in principle, but its ids are what those `person_id` columns hold, so it comes back from the dump)
- `priority_conflict_dismissals` (officer-acknowledged Priority List same-boss conflicts; losing it only makes a dismissed warning reappear, same low-stakes shape as `account_preferences`)
...
6. **Redeploy Edge Functions**: re-enter their secrets first (Project Settings > Edge Functions), then `gh workflow run deploy.yml -f functions=all` once step 8 has repointed the workflow, or `supabase functions deploy` by hand; either way the hold list in `scripts/ci/functions-to-deploy.js` still applies.
...
8. **Update the `SUPABASE_DB_URL` repo secret** to the new project's session pooler string so the nightly backup resumes against the new project, `PROJECT_REF` in `.github/workflows/deploy.yml` to the new ref, `PRODUCTION_HOST` in `supabase/functions/_shared/discord-destination.ts` to the new host (one of the places the ref lives; `git grep kxgjqnpwfklbgrxdgmmv` lists them all, and `tests/ci/functions-webhook-destination.test.js` pins this one against `js/common.js`), and `SUPABASE_EDGE_FUNCTIONS_DEPLOY_TOKEN` to a token whose account is a Developer on the new project. Until that constant moves, the restored project reads as a local stack and every Discord post lands in the test channel marked `[local]`, or is skipped if no test webhook is set.
9. **Auth relink.** The new project's `auth.users` starts empty, so every login is a first login; `link_auth_user_to_member()` re-links by `discord_id` on first login: it attaches the account to the `people` row holding that Discord id ([#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942)), which is how the guild-wide grants in `guild_grants` reach it, and fills `team_members.auth_user_id`. **It does not overwrite a stale id**: every branch ends `and auth_user_id is null`, so a restored row that still carries an id from the dead project is never relinked and the person silently keeps no access. Null those ids before the relinks land (`update <table> set auth_user_id = null`, `people` included, where a person left with no Discord id deletes itself), or nothing in this step does anything. Recreate the seven FKs from step 3 as `not valid` (their definitions are in the migrations) so historical `audit_log.actor_id` values survive, then `validate constraint` once relinks settle or stale ids are nulled. `wga-auth-<date>.dump` is the reference copy of the old ids and Discord identities if anything needs untangling by hand.
...
- **Table-count floor of 20** in the same verify step -- real count was 26 when written, 27 as of [#607](https://github.com/katogaming88/WGA-Raid-Hub/issues/607)'s `guild_officers` table, then 29 (`tier_token_map` from #650, `no_character_dismissals` from [#512](https://github.com/katogaming88/WGA-Raid-Hub/issues/512)), then 32 (`boe_items`, `boe_listings`, `boe_managers` from [#745](https://github.com/katogaming88/WGA-Raid-Hub/issues/745)), then 33 (`priority_conflict_dismissals`), then 36 (`player_equipped_gear` and `priority_order_confirmed_empty` landed the same day as `priority_conflict_dismissals` without updating this count, caught then; plus `priority_stale_dismissals`), then 46 (44 on production before [#1114](https://github.com/katogaming88/WGA-Raid-Hub/issues/1114) added `guilds` and `retired_url_keys`; the eight in between landed without updating this count), then 48 (`people` from [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942) and `seasons` from [#932](https://github.com/katogaming88/WGA-Raid-Hub/issues/932), both 2026-09-14), then 46 (#942 step 2 replaced `site_admins`, `guild_officers` and `boe_managers` with `guild_grants`, 2026-09-15), then 47 (`characters`, #942 step 5, 2026-09-15), then 48 (`main_swap_requests`, #631, 2026-09-16, without updating this count), then 51 (`boss_groups`, `raid_night_bosses`, `raid_night_lineups`, [#1216](https://github.com/katogaming88/WGA-Raid-Hub/issues/1216), 2026-09-18), then 52 (`team_lineup_settings`, [#1244](https://github.com/katogaming88/WGA-Raid-Hub/issues/1244), 2026-09-18), 51 (`bis_items` dropped, #935, 2026-09-20), then 52 (`team_seasons`, [#939](https://github.com/katogaming88/WGA-Raid-Hub/issues/939), 2026-09-21), now 53 (`team_invite_links`, [#1264](https://github.com/katogaming88/WGA-Raid-Hub/issues/1264), 2026-09-22) -- still well clear of the floor, no re-check needed.
- **The `EMPTY_CHECK` list** in the same verify step -- `players`, `item_preferences`, `season_signups`, `team_settings`, `team_members`, `teams`, `guild_grants`, `attendance`. Every name is a claim that the table cannot legitimately be empty. Renaming or dropping one of them makes the check fail as `(unreadable)` until the list is updated, which is deliberate: a silently skipped assertion is the failure this whole step exists to prevent.
- **The coverage map above** -- it is meant to classify all 52 base tables (eight are still unlisted, see the 2026-09-13 #1114 note below), so a new table leaves it incomplete without anything failing.
- **The seven `auth.users` FKs** in the full-rebuild runbook (steps 3 and 9) -- `people.auth_user_id` ([#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942)), `audit_log.actor_id`, `team_members.auth_user_id`, `season_signups.auth_user_id`, `account_preferences.auth_user_id` ([#940](https://github.com/katogaming88/WGA-Raid-Hub/issues/940), which replaced `no_character_dismissals` from [#512](https://github.com/katogaming88/WGA-Raid-Hub/issues/512)), `priority_conflict_dismissals.dismissed_by`, `priority_stale_dismissals.dismissed_by`. This list is the one to trust: steps 3 and 9 disagreed with it between #512 and #766, because #512 updated here and not there.
...
As of 2026-09-13, [#940](https://github.com/katogaming88/WGA-Raid-Hub/issues/940) replaced `no_character_dismissals` with `account_preferences`: one table dropped and one added, so the table count is unchanged, and the `auth.users` FK moved with it, so there are still nine. Classified in-app-only above, same low-stakes shape. No `EMPTY_CHECK` floor (a guild where nobody dismissed the prompt or cleared the bell has no rows). The FK list above and steps 3 and 9 name the new table. No re-drill due.

As of 2026-09-13, [#1114](https://github.com/katogaming88/WGA-Raid-Hub/issues/1114) added `guilds` and `retired_url_keys` (46 base tables; production had 44 before it, not the 36 this log last recorded), both classified in-app-only above, and two columns on existing backed-up tables (`teams.guild_id`, `players.url_code`, whose codes appear in shared links and so must come back from the dump rather than be regenerated). No new `auth.users` FK. Restore order gains one step: `guilds` before `teams`. No `EMPTY_CHECK` floor for `guilds`: `teams` already has one, and a team cannot exist without its guild. `retired_url_keys` is legitimately empty until a key is renamed. Checking the count turned up seven tables that landed since 2026-09-01 without a line in the coverage map: `raid_schedule`, `raid_schedule_exceptions`, `raid_rsvps`, `raid_rsvp_reminders_sent`, `raid_signup_sheets`, `team_discord_config` and `track_bonus_ids`. They are left for [#699](https://github.com/katogaming88/WGA-Raid-Hub/issues/699) rather than classified in passing here. No re-drill due.

As of 2026-09-14, [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942) added `people` (47 base tables) and a `person_id` column on `team_members`, `site_admins`, `guild_officers` and `boe_managers`, classified in-app-only above. A tenth `auth.users` FK, `people.auth_user_id`, joins the list above and steps 3 and 9. Restore order gains one step: `people` before the four grant tables. No `EMPTY_CHECK` floor: `team_members` and `site_admins` already have one, and neither can hold a row without its person. `scripts/dev/db-snapshot.js` nulls `people.auth_user_id` with the other links. No re-drill due.

As of 2026-09-14, [#932](https://github.com/katogaming88/WGA-Raid-Hub/issues/932) added `seasons` (48 base tables), classified regenerable above, and a foreign key from each of the fourteen `season` columns to it. No `auth.users` FK, no `EMPTY_CHECK` floor (its rows come from the migrations). A selective restore of a season-bearing table now needs the tier rows present, which a restored schema already has. No re-drill due.

As of 2026-09-15, [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942) step 2 replaced `site_admins`, `guild_officers` and `boe_managers` with `guild_grants` (46 base tables: three dropped, one added; the three names are now views of it). Their three `auth.users` FKs are gone, since a grant reaches its account through `people`, leaving seven in the list above and steps 3 and 9. Restore order: `people`, then `guild_grants`. The `EMPTY_CHECK` floor that was on `site_admins` moves to `guild_grants` in `db-backup.yml` and `scripts/dev/db-snapshot.js`, and `db-snapshot.js` only nulls the old tables' `auth_user_id` on a dump taken before this step. No re-drill due.

As of 2026-09-15, [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942) step 5 added `characters` (47 base tables), classified in-app-only above, pointing at `people` with `on delete cascade`. No `auth.users` FK. Restore order: `people`, then `characters`. No `EMPTY_CHECK` floor: nobody has picked any alts until the new app's picker ships. No re-drill due.

As of 2026-09-18, [#1216](https://github.com/katogaming88/WGA-Raid-Hub/issues/1216) added `boss_groups`, `raid_night_bosses` and `raid_night_lineups` (51 base tables), classified in-app-only above. `raid_night_bosses.confirmed_by` points at `people`, not `auth.users`, so no new `auth.users` FK. Restore order: `raid_night_bosses` before `raid_night_lineups` (a composite FK). No `EMPTY_CHECK` floor: a team that does not plan boss by boss has no rows. The count also caught `main_swap_requests` (#631, 2026-09-16), which landed without a line in the coverage map; left for [#699](https://github.com/katogaming88/WGA-Raid-Hub/issues/699) with the other seven. No re-drill due.
...
As of 2026-09-21, [#939](https://github.com/katogaming88/WGA-Raid-Hub/issues/939) added `team_seasons` (52 base tables), classified above with `team_settings`: officer-set switches per tier that nothing else records. No `EMPTY_CHECK` floor: a guild whose teams have opened nothing has no rows. No new `auth.users` FK. No re-drill due.

As of 2026-09-22, [#1264](https://github.com/katogaming88/WGA-Raid-Hub/issues/1264) added `team_invite_links` (53 base tables), classified in-app-only above: an officer's generated code and its expiry, nothing else records either. Its only FK is `team_id` to `teams`, already a restore root, so no new `auth.users` FK and no ordering change beyond restoring after `teams`. No `EMPTY_CHECK` floor: a team that has never made a link has no row, a legitimate state. No re-drill due.
