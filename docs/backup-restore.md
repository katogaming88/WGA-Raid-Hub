...
- `people` (one row per human, [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942); the four grant tables point at it through `person_id`, so it restores before them. Rebuildable from the grant tables' Discord ids and `auth.identities` in principle, but its ids are what those `person_id` columns hold, so it comes back from the dump)
...
9. **Auth relink.** The new project's `auth.users` starts empty, so every login is a first login; `link_auth_user_to_member()` re-links by `discord_id` on first login across all four grant tables (`team_members`, `site_admins`, `boe_managers` and, since #910, `guild_officers`), and since [#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942) attaches the account to the `people` row holding that Discord id. **It does not overwrite a stale id**: every branch ends `and auth_user_id is null`, so a restored row that still carries an id from the dead project is never relinked and the person silently keeps no access. Null those ids before the relinks land (`update <table> set auth_user_id = null`, `people` included, where a person left with no Discord id deletes itself), or nothing in this step does anything. Recreate the ten FKs from step 3 as `not valid` (their definitions are in the migrations) so historical `audit_log.actor_id` values survive, then `validate constraint` once relinks settle or stale ids are nulled. `wga-auth-<date>.dump` is the reference copy of the old ids and Discord identities if anything needs untangling by hand.
...
- **The ten `auth.users` FKs** in the full-rebuild runbook (steps 3 and 9) -- `people.auth_user_id` ([#942](https://github.com/katogaming88/WGA-Raid-Hub/issues/942)), `audit_log.actor_id`, `site_admins.auth_user_id`, `team_members.auth_user_id`, `season_signups.auth_user_id`, `guild_officers.auth_user_id`, `account_preferences.auth_user_id` ([#940](https://github.com/katogaming88/WGA-Raid-Hub/issues/940), which replaced `no_character_dismissals` from [#512](https://github.com/katogaming88/WGA-Raid-Hub/issues/512)), `boe_managers.auth_user_id` ([#766](https://github.com/katogaming88/WGA-Raid-Hub/issues/766)), `priority_conflict_dismissals.dismissed_by`, `priority_stale_dismissals.dismissed_by`. This list is the one to trust: steps 3 and 9 disagreed with it between #512 and #766, because #512 updated here and not there.
