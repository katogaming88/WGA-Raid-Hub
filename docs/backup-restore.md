...
8. **Update the `SUPABASE_DB_URL` repo secret** to the new project's session pooler string so the nightly backup resumes against the new project, `PROJECT_REF` in `.github/workflows/deploy.yml` to the new ref, and `SUPABASE_EDGE_FUNCTIONS_DEPLOY_TOKEN` to a token whose account is a Developer on the new project.
