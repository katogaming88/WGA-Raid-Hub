-- generate_priority_order(): grade the equipped-slot factor by the gear's
-- real upgrade track instead of its item level, and split it into three
-- steps. Companion to 20260913013949, which added track_bonus_ids and made
-- player_equipped_gear.track trustworthy; see that migration's header for
-- why the item-level guess could not work and what it measured.
--
-- Two separate things in this function read equipped gear, and only one of
-- them needed SQL changes here:
--
--   1. The "already owns this exact item" exclusion (recip CTE) reads
--      peg.track directly, so it is fixed simply by track now being real.
--      Its semantics are unchanged and deliberately stay rank-blind: owning
--      the item on the track being generated removes you at any upgrade rank
--      (1/6 through 6/6), and owning only the Hero copy still does not
--      remove you from a Myth generation -- that keeps the existing "Has
--      Heroic" score reduction instead.
--
--   2. The equipped-slot factor (this migration) previously asked "does any
--      item in this slot clear the Myth/Hero item-level floor", which is the
--      question the overlapping tracks make unanswerable. It now asks "what
--      track is this slot already itemized at", in three steps:
--        Myth in the slot     -> 0.92 (needs it least)
--        Hero in the slot     -> 0.96
--        Champion or lower,
--        or an empty slot     -> 1.00 (needs it most)
--
-- Why track and not item level, given a Champion 6/6 (308) can out-gear a
-- Hero 1/6 (305): the factor exists to reflect who gets the most value from
-- the drop, and that is driven by upgrade headroom and crest cost, not
-- current power. A player capped at Hero 6/6 cannot improve that slot
-- further without a new item, while a player sitting on Myth 1/6 can upgrade
-- what they already have -- so the Hero player should out-rank them for a
-- Myth drop even though their current item is the stronger one. Grading by
-- item level scores those two identically (321 and 318 both clear a 318
-- floor) and loses that entirely. The known cost is that Champion 6/6 vs
-- Hero 1/6 is graded the "wrong" way round on raw power; accepted as not
-- worth the extra machinery, since this factor only nudges a candidate
-- within the list and never removes anyone, and officers see and can reorder
-- the whole list.
--
-- Rings and trinkets grade off the LOWER track of their two slots. Those
-- slots are a pair a player fills twice, so someone wearing one Myth and one
-- Champion trinket still genuinely wants a trinket -- the drop would replace
-- the Champion one. The previous bool_or() took the better of the two and
-- penalized them as fully Myth-itemized. An unfilled second slot counts as
-- the lowest step for the same reason.
--
-- Weapons deliberately stay single-slot (One-Hand/Two-Hand/Ranged ->
-- MAIN_HAND, Off Hand -> OFF_HAND) rather than being paired like rings and
-- trinkets. Whether the two are a pair depends on whether the player
-- dual-wields, which the schema does not know -- an OFF_HAND row may hold a
-- second weapon, a shield, or an off-hand frill, and pooling would be wrong
-- for two of those three. The app already made this same call deliberately
-- elsewhere (WISHLIST_SIBLING_SLOT in js/wishlist.js exempts Weapon/Off Hand
-- from the ring/trinket sibling handling, because a dual-wield class can
-- legitimately want two copies of one item).
